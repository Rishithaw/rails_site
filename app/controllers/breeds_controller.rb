class BreedsController < ApplicationController
  ...
  def index
    @breeds = Breed.includes(:sub_breeds).page(params[:page]).per(6)
  end
  ...
  def update
    respond_to do |format|
      if @breed.update(breed_params)
        format.html { redirect_to @breed, notice: "Breed was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to breeds_path, notice: "Breed was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_breed
      @breed = Breed.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def breed_params
      params.expect(breed: [ :name ])
    end
end
